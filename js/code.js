...
timeInput.value = new Date().toLocaleTimeString(navigator.language, {
  hour: "2-digit",
  minute: "2-digit",
});
...
function choseLeftBreast(e) {
  ...
  changeBreast("prawa");
  renderFeeds();
  addDataToFirebaseWithDateAsDoc(
    "lewa",
...
const editRecord = async function (e) {
  ...
  if (
    leftBreastButtonEdit.textContent.toLowerCase() ===
    this.querySelector("p:first-of-type").textContent.toLowerCase()
  ) {
  ...
  dateTimeInputEdit.value = this.querySelector(
    "p:last-of-type"
  ).textContent.replace(" ", "T");
  const container = document.createElement("div");
  container.classList.add("editRecord");
  container.append(
    leftBreastButtonEdit,
    rightBreastButtonEdit,
    dateTimeInputEdit
  );
